...
class ConcreteThermoMechanicalReinforced2D(MaterialProblem):
    ...
    def setup(self):
        ...
        default_p['E_reinforcement'] = 30000000 # in Pa
        default_p['A_reinforcement'] = 0.01 # m^2
...
class ConcreteMechanicsModel(df.NonlinearProblem):
    ...
    def set_reinforcement(self, experiment):
        ...
        H = self.p.height
        Ny = experiment.n_height
        A_eff = self.p.A_reinforcement/self.p.width
        E_s = self.p.E_reinforcement
        V = self.V
...
class Reinforcement:
    def __init__(self, params):
        list_1, L, Nx, H, Ny, A_area, E_s, V = params
        self.dofs = []
        self.list_1 = list_1
        self.L = L
        self.Nx = Nx
        ...
        self.E_s = E_s
        self.V = V
        self.assign_dofs(list_1, L, Nx,
                         xbar=True)  # this is for reinforcement bars in x direction. parameter "xbar" necessary to correctly assign x and y in rebar function
        # self.assign_dofs(list_2,H,Ny,xbar=False) # this is for reinforcement bars in y direction. parameter "xbar" necessary to correctly assign x and y in rebar function

    def assign_dofs(self, list_1, L, Nx, xbar):
        for list_x in list_1:
            for x_start, x_end, y in list_x:
                x_values = np.linspace(x_start, x_end, int(Nx * (x_end - x_start) / L) + 1)
                for i, x in enumerate(x_values):
                    ...
                    for i in [x_start, x_end]:
                        def rebar(x, on_boundary):
                            if xbar == True:
                                return df.near(x[1], y) and df.near(x[0], i)
                            if xbar == False:
                                return df.near(x[1], i) and df.near(x[0], y)

                        k = 0 if xbar == True else 1
                        self.dofs.append(list((df.DirichletBC(self.V.sub(k), df.Constant((0.)), rebar,
                                                              method="pointwise").get_boundary_values().keys()))[0])
                        # print(list((df.DirichletBC(V.sub(k), df.Constant((0.)), rebar,method="pointwise").get_boundary_values().keys()))[0])

    def to_apply(self, A):
        length_L = self.L / self.Nx  # length of a single reinforcement element in x direction
        AEL = self.A_area * self.E_s / length_L  # local stiffness for steel that will be added to the corresponding stiffness of concrete
        local_stiffness = np.array([[AEL, -AEL, -AEL, AEL]])
        for i, dof in enumerate(self.dofs):
            if i == len(self.dofs) - 1:
                break
            dof_array = np.array([dof, self.dofs[i + 1]], dtype=np.int32)
            if dof == self.dofs[i + 1]:  # otherwise you will add the values twice in the stiffness matrix
                continue
            A_petsc = df.as_backend_type(A).mat()
            A_petsc.setOption(PETSc.Mat.Option.NEW_NONZERO_ALLOCATION_ERR,
                              False)  # otherwise no new values can be added to new entries
            A_petsc.setValues(dof_array, dof_array, local_stiffness.flat, addv=PETSc.InsertMode.ADD)
            A_petsc.assemble()
